...
function StatCard({ icon: Icon, label, value, color, href }) {
  return (
    <Link href={href} className="group flex items-center gap-4 p-5 rounded-2xl bg-white dark:bg-slate-800 border border-gray-100 dark:border-slate-700 hover:border-violet-200 dark:hover:border-violet-700 hover:shadow-lg transition-all">
      ...
      <FiArrowRight size={16} className="ml-auto text-gray-300 dark:text-gray-600 group-hover:text-violet-500 transition-colors" />
    </Link>
  );
}
  ...
  return (
    <div>
      <div className="mb-8">
        ...
        <p className="text-gray-500 dark:text-gray-400 mt-1 text-sm">Here's what's happening with your account.</p>
      </div>
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/dashboard/buyer/orders" className="text-sm text-violet-600 dark:text-violet-400 font-medium hover:underline">View all</Link>
        </div>
        {loading ? (
          <div className="space-y-3">{[1,2,3].map(i => <div key={i} className="h-14 rounded-xl bg-gray-100 dark:bg-slate-700 animate-pulse" />)}</div>
        ) : orders.length === 0 ? (
          <div className="text-center py-10">
            <FiShoppingBag size={36} className="mx-auto text-gray-300 dark:text-slate-600 mb-3" />
            <p className="text-gray-500 dark:text-gray-400 text-sm">No orders yet. Start shopping!</p>
            <Link href="/" className="mt-3 inline-flex items-center gap-1.5 text-sm text-violet-600 dark:text-violet-400 font-medium hover:underline">Browse listings <FiArrowRight size={14} /></Link>
          </div>
        ) : (
              ...
              <thead>
                <tr className="border-b border-gray-100 dark:border-slate-700">
                  {["Product","Price","Status","Date"].map(h => (
                    <th key={h} className="pb-3 text-left text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-wide">{h}</th>
                  ))}
                </tr>
              </thead>
              <tbody className="divide-y divide-gray-50 dark:divide-slate-700">
                {orders.slice(0, 5).map(o => (
                  <tr key={o._id} className="hover:bg-gray-50 dark:hover:bg-slate-700/30 transition-colors">
                    ...
                    <td className="py-3"><span className={`px-2.5 py-0.5 rounded-full text-xs font-semibold ${
