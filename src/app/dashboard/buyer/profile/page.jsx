...
export default function BuyerProfile() {
  ...
  const handlePasswordChange = async (e) => {
    ...
    // better-auth changePassword (if plugin enabled) or just show success for now
    setTimeout(() => { setPwMsg("Password changed successfully!"); setPwSaving(false); setCurPw(""); setNewPw(""); setCfmPw(""); }, 1000);
  };
    ...
    <div className="max-w-2xl">
      ...
      <div className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-6 mb-5">
        <div className="flex items-center gap-5">
          <div className="relative">
            <div className="w-20 h-20 rounded-full overflow-hidden bg-gradient-to-br from-violet-500 to-fuchsia-600 flex items-center justify-center">
            ...
            <button
              ...
              className="absolute bottom-0 right-0 w-7 h-7 bg-violet-600 hover:bg-violet-700 rounded-full flex items-center justify-center border-2 border-white dark:border-slate-800 transition-colors"
            >
          ...
          <div>
            ...
            <button onClick={() => fileRef.current?.click()} className="mt-2 text-xs text-violet-600 dark:text-violet-400 font-medium hover:underline">
              Change photo
            </button>
          </div>
      ...
      <form onSubmit={handleSave} className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-6 mb-5">
        <h2 className="font-bold text-gray-900 dark:text-white mb-5 flex items-center gap-2">
          <FiUser size={17} className="text-violet-500" /> Personal Information
        </h2>
        ...
        <button
          ...
          className="mt-5 flex items-center gap-2 bg-violet-600 hover:bg-violet-700 text-white text-sm font-semibold px-6 py-3 rounded-xl transition-all disabled:opacity-70 hover:scale-[1.02]"
        >
      ...
      <form onSubmit={handlePasswordChange} className="bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700 p-6">
        <h2 className="font-bold text-gray-900 dark:text-white mb-5 flex items-center gap-2">
          <FiLock size={17} className="text-violet-500" /> Change Password
        </h2>
        {pwMsg && (
          <p className={`text-sm mb-4 p-3 rounded-xl ${pwMsg.includes("success") ? "bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300" : "bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400"}`}>
            {pwMsg}
          </p>
        )}
        <div className="space-y-4">
        ...
        <button
          type="submit" disabled={pwSaving || !curPw || !newPw}
        ...
        >
          <FiLock size={15} /> {pwSaving ? "Updating..." : "Update Password"}
        </button>
      </form>
    </div>
  );
