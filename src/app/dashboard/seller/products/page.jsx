        ...
        >
          ...
          <div className="flex gap-3 pt-2">
            ...
            <button
              type="button"
              onClick={onClose}
              className="px-5 border border-gray-200 dark:border-slate-600 text-gray-600 dark:text-gray-300 font-medium text-sm rounded-xl hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors"
            >
              Cancel
            </button>
          </div>
      ...
      {loading ? (
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {[1, 2, 3, 4, 5, 6].map((i) => (
            <div
              key={i}
              className="h-56 rounded-2xl bg-gray-100 dark:bg-slate-800 animate-pulse"
            />
          ))}
        </div>
      ) : visible.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 bg-white dark:bg-slate-800 rounded-2xl border border-gray-100 dark:border-slate-700">
          ...
          <p className="text-gray-400 dark:text-gray-500 text-sm mb-5">
            Start listing items to sell on ResellHub.
          </p>
          <Link
            href="/dashboard/seller/add-product"
